...
var noAcptFolderImg = "/ams/img/IconNoAcpt.gif" ;  // 접수
var acptFolderImg = "/ams/img/IconAcpt.gif" ;  // 접수
var rjctFolderImg = "/ams/img/IconRjct.gif" ;  // 반려
var cpltFolderImg = "/ams/img/IconCplt.gif" ;  // 인수
var inspFolderImg = "/ams/img/IconInsp.gif" ;  // 검수

var selectNoAcptFolderImg = "/ams/img/IconNoAcptOn.gif" ;  // 접수
var selectAcptFolderImg = "/ams/img/IconAcptOn.gif" ;  // 접수
var selectRjctFolderImg = "/ams/img/IconRjctOn.gif" ;  // 반려
var selectCpltFolderImg = "/ams/img/IconCpltOn.gif" ;  // 인수
var selectInspFolderImg = "/ams/img/IconInspOn.gif" ;  // 검수
...
/* 처리과 상태표시를 위한 것. */
function FolderForDept(id, parentId, name, url, childCnt, endFl,codeCl)
{
	this.id = id;
	this.parentId = parentId;			// 부모 ID
	this.name = name;	// 카테고리명
	...
	if(endFl == "0")
	{
		if(codeCl == "2")
		{
			this.selectIconImg = selectAcptFolderImg;	// 카테고리(폴더) 이미지
			this.IconImg = acptFolderImg;	// 카테고리(폴더) 이미지
		}
		else if(codeCl == "4")
		{
			this.selectIconImg = selectInspFolderImg;	// 카테고리(폴더) 이미지
			this.IconImg = inspFolderImg;	// 카테고리(폴더) 이미지
		}
		else if(codeCl == "5")
		{
			this.selectIconImg = selectRjctFolderImg;	// 카테고리(폴더) 이미지
			this.IconImg = rjctFolderImg;	// 카테고리(폴더) 이미지
		}
		else if(codeCl == "6")
		{
			this.selectIconImg = selectCpltFolderImg;	// 카테고리(폴더) 이미지
			this.IconImg = cpltFolderImg;	// 카테고리(폴더) 이미지
		}
		else
		{
			this.selectIconImg = selectNoAcptFolderImg;	// 카테고리(폴더) 이미지
			this.IconImg = noAcptFolderImg;	// 카테고리(폴더) 이미지
		}
	}
	else
	{
...
{
	parent.ListArea.document.location.href = url;
...
* @ url : 카테고리(폴더) 링크 정보
* @ openStat : 카테고리(폴더) 열고 닫기 상태(0, 1)
* @ Modify 생산현황, 인수기록물 관련 Tree를 표시 한다. 또한 OneClick와 CheckBox과 동시에 존재한다.
* @ Modify 처리과에 상태에 관련된 CodeCl 추가.

*/
function addCategoryForDHDept(id, parentId, ctgryNm, ctgryValue, checkYn, url, openStat, childCnt, seq, level, endFl, docDeptFl, codeCl)
{
	...
	// 노드 클릭시 해당 카테고리(폴더)의 정보를 사용하기 위해서
	// 추가하는 카테고리마다 폴더를 생성해서 배열로 보관한다.
	var category = new FolderForDept(id, parentId, ctgryNm, url, childCnt, endFl,codeCl);
	...
	if(openStat == 1)
	{
		...
		if(endFl == "0") // 현존부서
		{
			if(codeCl == "2")
			{
				...
				folderImg = acptFolderImg;	
			}
			else if(codeCl == "4")
			{
				
				folderImg = inspFolderImg;	// 카테고리(폴더) 이미지
			}
			else if(codeCl == "5")
			{
			
				folderImg = rjctFolderImg;	// 카테고리(폴더) 이미지
			}
			else if(codeCl == "6")
			{
				
				folderImg = cpltFolderImg;	// 카테고리(폴더) 이미지
			}
			else
			{
				folderImg = noAcptFolderImg;
			}
		}
		else // 폐지부서
		{
	...
	if(childCnt != "0")	
	{
		desc += " <a class=Table active href=\"javascript:OnClickNode('"+id+"', '"+ this.idNm +"')\"><img id=nodeIcon"+divId+" src=\""+ nodeImg +"\" border=\"0\" style='vertical-align:middle'></a>";
	}
	else
	{
		if(level > 1)
	...
	else
	{
		...
		if(docDeptFl == "Y")
		{
			checkboxCtgryNm = "<input type='checkbox' id='"+id.substr(1)+"' name='category' "+checkFlag+ " value=\""+ctgryValue+"\" onClick=\"javascript:toggleCheck(this)\" style='vertical-align:middle;height:16px'><a href=\"javascript:onClickCategoryForDHDept('"+url+"','"+divId+"', '"+endFl+"', '"+codeCl+"', '"+docDeptFl+"')\"> "+ ctgryNm +"("+id.substr(1)+")</a>"; // 체크박스가 들어간 팝업창 카테고리 명
		}
		else
		{
			checkboxCtgryNm = "<a class=Table active href=\"javascript:onClickCategoryForDHDept('"+url+"','"+divId+"', '"+endFl+"', '"+codeCl+"', '"+docDeptFl+"')\"> "+ ctgryNm +"("+id.substr(1)+")</a>";
		}
		desc += checkboxCtgryNm;
	}
...
*	폴더 클릭함수 (url호출)
*/
function onClickCategoryForDHDept(url, divId, endFl,codeCl,docDeptFl)
{
	...
	if(endFl == "0") // 현존부서
	{
		...
		if(codeCl == "2")
		{
			document.getElementById("folderIcon" + divId).src = selectAcptFolderImg;
		}
		else if(codeCl == "4")
		{
			document.getElementById("folderIcon" + divId).src = selectInspFolderImg;
		}
		else if(codeCl == "5")
		{
			document.getElementById("folderIcon" + divId).src = selectRjctFolderImg;
		}
		else if(codeCl == "6")
		{
			document.getElementById("folderIcon" + divId).src = selectCpltFolderImg;
		}
		else 
		{
			document.getElementById("folderIcon" + divId).src =  selectNoAcptFolderImg;
		}
	}
	else
	{
	...
	if(docDeptFl == "N" || codeCl == "2" || codeCl == "3"  ||  codeCl == "4" ||  codeCl == "6" )
	{
		parent.parent.WorkingArea.document.location.href = url;	
	}
	else
	{
		var message = "접수하시겠습니까?"
		if(confirm(message))
		{
			popupWin(url, "PopUp", "580", "350"); 
		}
...
var tmpTFIMG = new Image();
var PreviousNode = "";
var PreviousNodeImage = "";
function setSelectedImage(target, selectedImag, targetImage)
{
	
	tmpTFIMG.src = selectedImag;
	target = 'I'+target;
	var CI = document.images[target];	
	if (!CI) return;
	CI.src = tmpTFIMG.src;
	if ( PreviousNode.length > 0 && PreviousNode != target )
	{
		if ( document.images[PreviousNode] )
		{
			var PI = document.images[PreviousNode];
