...
function CTreeNode(treeView, parentNode , text, url, target){
	...
	this.moveTo=function(x, y){if (this.treeView.ns4)this.el.moveTo(x,y);else{var px=this.treeView.operaOld?0:'px';this.el.style.left=x+px;this.el.style.top=y+px;}}
...
function CTreeBack(aleft, atop, color, name){
	...
	this.moveTo=function(x, y){if (this.ns4)this.el.moveTo(x,y);else{var px=this.bw.operaOld?0:'px';this.el.style.left=x+px;this.el.style.top=y+px;this.el2.style.left=x+px;}};
	this.resize=function(w,h){if (this.ns4){this.el.resizeTo(w,h);}else{var px=this.bw.operaOld?0:'px';this.el.style.width=w+px;this.el.style.height=h+px;if (this.r) {this.el2.style.top=(h+this.top-5)+px;}}};
