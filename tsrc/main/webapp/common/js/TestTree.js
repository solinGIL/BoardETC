...
var selectDelFolderImg = "/ams/img/IconTreeEOn.gif";	// 삭제부서의 폴더 이미지(열렸을 경우)
var	delFolderImg  = "/ams/img/IconTreeEOff.gif";	// 삭제부서의 폴더 이미지(닫혔을 경우)
var	funcFolderImg = "/ams/img/iconFunc.gif";// 기능폴더
var	selectFuncFolderImg = "/ams/img/IconOn.gif"; // 기능폴더

var menuTreeConfig = {
	
	folderIcon      : '/ams/img/iconFunc.gif',
	openFolderIcon  : '/ams/img/IconOn.gif',
	iIcon           : '/ams/img/IconTreeIn.gif',
	lIcon           : '/ams/img/IconTreeLn.gif',
	lMinusIcon      : '/ams/img/IconMinus.gif',
	lPlusIcon       : '/ams/img/IconPlus.gif',
	tIcon           : '/ams/img/IconTreeAn.gif',
	tMinusIcon      : '/ams/img/IconMinus.gif',
	tPlusIcon       : '/ams/img/IconPlus.gif',
	blankIcon       : '/ams/img/IconTreeD.gif',
...
MenuTreeAbstractNode.prototype.indent = function(lvl, del, last, level, nodesLeft) {
	...
	if (foo) {
		...
		else { foo.src =  menuTreeConfig.iIcon; }
	}
	return 0;
}
...
function MenuTreeItem(sText, sId, sValue, sAction, blink, check,  funcCl, codeCl, eParent) {
	this.base = MenuTreeAbstractNode;
...
MenuTreeItem.prototype.toString = function (nItem, nItemCount) {
	...
	while (foo.parentNode) {
		foo = foo.parentNode;
		indent = "<img id=\"" + this.id + "-indent-" + i + "\" src=\"" + ((foo._last || !foo.parentNode)?menuTreeConfig.blankIcon:menuTreeConfig.iIcon) + "\">" + indent;
		i++;
	}
	// 트리 모양을위해 다시 셋팅
	foo = this.parentNode;
	...
	//alert(this.id+ "-icon");
	if(this.check == true)
		str +="<input type='checkbox' id='"+this.id+"' name=\"category\"  value=\""+this.sValue+"\"  style='vertical-align:middle;height:16px'>"; // 체크박스가 들어간 팝업창 카테고리 명
	...
	str += "<a class=\"menu-tree-icon\"  href=\"javascript:onClickCategory('"+this.id+"','"+this.openIcon+"','"+this.action+"','"+this.sValue+"');\" id=\"" + this.id + "-anchor\"    onfocus=\"blur();\">&nbsp;"+  label + "</a></div>";
...
function onClickCategory(id, openIcon, action, value)
{
	this.id = id ;
	this.action = action ;
	this.value = value ;
